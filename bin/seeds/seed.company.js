...
const companies = [
  ...
  },
  {
    name: "Botify",
    logo:
      "https://media-exp1.licdn.com/dms/image/C4D0BAQHgwUch43UKKA/company-logo_200_200/0/1579822106102?e=1625702400&v=beta&t=4CLNotDyaOxEImyqoiw4F35FwV6xp9YkQW4FhAftqtc",
    city: "New York",
    website: "botify.com",
    industry: "SaaS",
    size: 500,
...
(async function insertCompanies() {
  try {
    ...
    // Attribute 1 recruiter to the 2 first companies
    companies[0].users = [];
    companies[0].users.push(allRecruiters[0]._id);
    companies[1].users = [];
    companies[1].users.push(allRecruiters[1]._id);

    // Attributes recruiter[2] to companies[1]
    companies[1].users.push(allRecruiters[2]._id);

    // Attribute all other companies to recruiter[2]
    for (let i = 2; i < allRecruiters.length; i++) {
      companies[i].users = [];
      companies[i].users.push(allRecruiters[2]._id);
    }
